fetch('/admin/usersData', {
    ...
    let pagesCount = Math.ceil(users.length / 15);
    let currentPage = 1;
    // create list btns
    ...
    let usersList = users.slice(startIndex, endIndex);
                ...
                <div>
                    <form action="/admin/deleteUser/${users[i]._id}" method="POST"
                        ...
                        <a href="/admin/users/userDetails/${users[i]._id}">
                            <img src="/images/admin-img/show.png"
    ...
    function showProducts() {
        usersList = users.slice(startIndex, endIndex);
        usersBody.innerHTML = "";
